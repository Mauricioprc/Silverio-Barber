...
9. **Quadro de sócios é fixo em 2, sem fluxo de expansão automatizado**: `registrar-socio`
   aceita livremente os 2 primeiros cadastros e recusa (403) a partir do 3º, sem nenhuma
   rota alternativa de aprovação. O modelo de negócio (ver `planejamento-geral.md`) é de 2
   sócios fixos; não há previsão de expansão do quadro societário. Se isso um dia for
   necessário, é tratado como ação administrativa pontual (acesso direto ao banco ou
   migração específica àquela época), não como infraestrutura construída preventivamente.
